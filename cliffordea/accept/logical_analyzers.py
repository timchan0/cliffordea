...
from cliffordea.accept.probability import (
    ...
    unencoded_trivial_syndrome_probability,
)
...
class CliffordLogicalAnalyzer(LogicalAnalyzer):
    ...
    def __init__(
        ...
        self.z_precheck_generators = tuple(
            pauli_masks_and_j_power(generator)[:2]
    ...
    def analyze(self, cultivated_state, before_transversal):
        ...
        if (
            ...
            accept_probability = evaluate_acceptance_structure(
                structure,
                effect_z,
            )
        ...
        accept_probability = unencoded_trivial_syndrome_probability(
            unencoded_error,
            stabilizer_rank=self.stabilizer_rank,
            logical_coefficients=LOGICAL_COEFFICIENTS[cultivated_state],
            checked_stabilizer_count=checked_stabilizer_count,
        )
    ...
    def _build_transversal_structure(
        ...
        for stabilizer_index in range(checked_stabilizer_count):
            ...
            columns.append(pauli_masks_and_j_power(transformed_generator))
            physical_generator = self.encoder(transformed_generator)
            physical_x_mask, _, _ = pauli_masks_and_j_power(
                physical_generator,
            )
            phase_masks.append(physical_x_mask)
        return build_acceptance_structure(
            columns=columns,
            ...
            unencoded_logical_mask=self.unencoded_logical_mask,
        )
