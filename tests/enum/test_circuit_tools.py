...
from cliffordea.enum.types import ErrorEvent
...
def test_batched_error_event_insertion_matches_sequential_insertion():
    ...
    for events, probability in event_groups:
        sequential = circuit_tools.insert_error_events(
            circuit=sequential,
    ...
    slices = circuit_tools.split_by_ticks(circuit)
    locations = circuit_tools.measurement_locations_by_event(slices)
    assert locations == {
        (0, (0,)): (0, 0),
        (0, (1,)): (0, 1),
        (0, (2,)): (1, 0),
    ...
    batched = circuit_tools.insert_error_events(
        circuit=circuit,
        weighted_error_events=(
